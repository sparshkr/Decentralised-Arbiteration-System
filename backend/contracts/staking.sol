...
contract Staking {
    ...
    constructor(IERC20 _grullToken, uint256 _minStakeAmount, uint256 _jurySize) {
        owner = msg.sender;
    ...
    modifier onlyDuringStaking(uint256 _disputeId) {
        require(block.timestamp < disputes[_disputeId].stakingEndTime, "Staking period has ended");
        _;
    }

    modifier onlyDuringVoting(uint256 _disputeId) {
        require(block.timestamp >= disputes[_disputeId].stakingEndTime, "Staking period is ongoing");
        require(block.timestamp < disputes[_disputeId].voteEndTime, "Voting period has ended");
        _;
    }

    modifier onlyAfterVoting(uint256 _disputeId) {
        require(block.timestamp >= disputes[_disputeId].voteEndTime, "Voting is still open");
        _;
    }

    // Create a new dispute
    function createDispute(string memory _sideA, string memory _sideB, uint256 _stakingDuration, uint256 _votingDuration) public onlyOwner {
        disputeCount++;

        disputes[disputeCount].sideA = _sideA;
        disputes[disputeCount].sideB = _sideB;
        disputes[disputeCount].stakingEndTime = block.timestamp + _stakingDuration;
        disputes[disputeCount].voteEndTime = disputes[disputeCount].stakingEndTime + _votingDuration;
    ...
    function stakeTokens(uint256 _disputeId, uint256 _amount) public onlyDuringStaking(_disputeId) {
        require(_amount >= minStakeAmount, "Stake must meet minimum requirement");
    ...
    function selectJury(uint256 _disputeId) public onlyOwner onlyDuringVoting(_disputeId) {
        Dispute storage dispute = disputes[_disputeId];
        ...
        for (uint256 i = 0; i < dispute.jurorAddresses.length; i++) {
            totalStakedAmount += dispute.jurors[dispute.jurorAddresses[i]].stakedAmount;
        }

        // Randomly select jurors based on staking amount
        for (uint256 i = 0; i < jurySize; i++) {
            uint256 randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, i))) % dispute.jurorAddresses.length;
    ...
    // Cast a vote
    function vote(uint256 _disputeId, VoteType _vote) public onlyDuringVoting(_disputeId) {
        require(_vote != VoteType.None, "Invalid vote");
        Dispute storage dispute = disputes[_disputeId];
        require(!dispute.jurors[msg.sender].hasVoted, "Already voted");
        require(isSelectedJuror(_disputeId, msg.sender), "Not selected as a juror");
    ...
    function isSelectedJuror(uint256 _disputeId, address _juror) internal view returns (bool) {
        Dispute storage dispute = disputes[_disputeId];
    ...
    // Resolve the dispute
    function resolveDispute(uint256 _disputeId) public onlyOwner onlyAfterVoting(_disputeId) {
        Dispute storage dispute = disputes[_disputeId];
        ...
        for (uint256 i = 0; i < dispute.selectedJury.length; i++) {
            if ((isSideAWinner && dispute.votes[dispute.selectedJury[i]] == VoteType.A) || 
                (!isSideAWinner && dispute.votes[dispute.selectedJury[i]] == VoteType.B)) {
                totalWinningStake += dispute.jurors[dispute.selectedJury[i]].stakedAmount;
            } else {
                totalLosingStake += dispute.jurors[dispute.selectedJury[i]].stakedAmount;
            }
        }
        ...
        for (uint256 i = 0; i < dispute.selectedJury.length; i++) {
            if ((isSideAWinner && dispute.votes[dispute.selectedJury[i]] == VoteType.A) || 
                (!isSideAWinner && dispute.votes[dispute.selectedJury[i]] == VoteType.B)) {
                uint256 jurorStake = dispute.jurors[dispute.selectedJury[i]].stakedAmount;
                uint256 reward = jurorStake + (jurorStake * totalLosingStake / totalWinningStake);
        ...
        for (uint256 i = 0; i < dispute.jurorAddresses.length; i++) {
            if (!isSelectedJuror(_disputeId, dispute.jurorAddresses[i])) {
                grullToken.transfer(dispute.jurorAddresses[i], dispute.jurors[dispute.jurorAddresses[i]].stakedAmount);
            }
        }
